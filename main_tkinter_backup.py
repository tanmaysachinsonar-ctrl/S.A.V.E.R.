...
class SAVERApp(ctk.CTk):
    def __init__(self):
        ...
        threading.Thread(target=self._ui_capture_loop,        daemon=True).start()
    ...
    def _build_home(self):
        ...
        if saved:
            ...
            ctk.CTkButton(wrap, text="⚙   Konfiguration einrichten",
                          ...
                          fg_color=GREEN, hover_color="#00aa55", text_color="#ffffff",
                          command=self._build_setup).pack(fill="x")
        ctk.CTkButton(wrap, text="📱   QR-Code – Handy-Remote öffnen",
                      font=ctk.CTkFont("Segoe UI", 14), height=40, corner_radius=12,
                      fg_color="#0d0d2a", hover_color="#181835", text_color="#6688cc",
                      border_width=1, border_color="#2a2a44",
                      command=self._open_qr_popup).pack(fill="x", pady=(12, 0))
    ...
    def _build_main(self):
        ...
        ctk.CTkButton(top, text="📱", width=40, height=36, font=ctk.CTkFont("Segoe UI", 18),
                      fg_color="transparent", hover_color="#1a1a33", text_color="#667788",
                      command=self._open_qr_popup).pack(side="right", padx=(0, 2))
    ...
    # ── QR CODE POPUP ─────────────────────────────────────────────────────────
    def _open_qr_popup(self):
        from config import _get_local_ip
        from config import STREAM_PORT
        url = f"http://{_get_local_ip()}:{STREAM_PORT}"
        win = ctk.CTkToplevel(self)
        win.title("📱  Handy-Remote")
        win.geometry("360x480")
        win.configure(fg_color=BG)
        win.transient(self)
        win.grab_set()
        _center_toplevel(win, self)
        ctk.CTkLabel(win, text="📱  Handy-Remote",
                     font=ctk.CTkFont("Segoe UI", 20, "bold"), text_color=BLUE).pack(pady=(18, 4))
        ctk.CTkLabel(win, text="Scanne den QR-Code mit deinem iPad / Smartphone",
                     font=ctk.CTkFont("Segoe UI", 12), text_color=MUTED,
        ...
        try:
            ...
            qr.add_data(url)
            qr.make(fit=True)
            qr_img = qr.make_image(fill_color="white", back_color="#0e0e20").convert("RGB")
            ctk_img = ctk.CTkImage(light_image=qr_img, dark_image=qr_img, size=(240, 240))
            ctk.CTkLabel(win, image=ctk_img, text="").pack(pady=(0, 10))
        except Exception:
            ctk.CTkLabel(win, text="qrcode-Paket nicht verfügbar",
                         font=ctk.CTkFont("Segoe UI", 12), text_color="#ff5555").pack()
        ctk.CTkLabel(win, text=url,
                     font=ctk.CTkFont("Segoe UI", 13, "bold"), text_color=BLUE).pack(pady=(0, 4))
        ctk.CTkLabel(win, text="PC und Handy müssen im gleichen WLAN sein",
                     font=ctk.CTkFont("Segoe UI", 11), text_color=MUTED).pack()
        ctk.CTkButton(win, text="✕  Schließen", width=120, height=36,
                      font=ctk.CTkFont("Segoe UI", 13),
                      fg_color=CARD, hover_color="#1e1e3a", text_color="#8899aa",
                      command=lambda: (win.grab_release(), win.destroy())).pack(pady=14)

    # ── WEB-REMOTE SCREENSHOT ─────────────────────────────────────────────────
    def _ui_capture_loop(self):
        import ctypes
        ...
        while True:
            try:
                ...
                # Client-Bereich ohne Titelleiste/Rahmen
                cr = wt.RECT()
                user32.GetClientRect(hwnd, ctypes.byref(cr))
                bw = cr.right  - cr.left
                bh = cr.bottom - cr.top
                if bw > 100 and bh > 100:
                    # PrintWindow (PW_CLIENTONLY=1) rendert direkt aus dem
                    # Fenster-Speicher – überlagernde Fenster stören nicht
                    hdc_win = user32.GetDC(hwnd)
                    hdc_mem = gdi32.CreateCompatibleDC(hdc_win)
                    hbmp    = gdi32.CreateCompatibleBitmap(hdc_win, bw, bh)
                    gdi32.SelectObject(hdc_mem, hbmp)
                    user32.PrintWindow(hwnd, hdc_mem, 1)   # 1 = PW_CLIENTONLY

                    bmi = BITMAPINFOHEADER()
                    bmi.biSize        = ctypes.sizeof(BITMAPINFOHEADER)
                    bmi.biWidth       = bw
                    bmi.biHeight      = -bh   # negativ → top-down
                    bmi.biPlanes      = 1
                    bmi.biBitCount    = 32
                    bmi.biCompression = 0     # BI_RGB

                    buf_raw = (ctypes.c_uint8 * (bw * bh * 4))()
                    gdi32.GetDIBits(hdc_mem, hbmp, 0, bh,
                                    buf_raw, ctypes.byref(bmi), 0)
                    gdi32.DeleteObject(hbmp)
                    gdi32.DeleteDC(hdc_mem)
                    user32.ReleaseDC(hwnd, hdc_win)

                    arr = np.frombuffer(buf_raw, dtype=np.uint8).reshape((bh, bw, 4))
                    bgr = arr[:, :, :3].copy()   # BGRA → BGR

                    ok, enc = cv2.imencode(".jpg", bgr, [cv2.IMWRITE_JPEG_QUALITY, 80])
                    if ok:
                        # Client-(0,0) in Bildschirmkoordinaten für Click-Mapping
                        pt = wt.POINT()
                        pt.x = 0; pt.y = 0
                        user32.ClientToScreen(hwnd, ctypes.byref(pt))
                        with _lock:
                            g["ui_screenshot"] = enc.tobytes()
                            g["ui_win_pos"]    = (pt.x, pt.y)
                            g["ui_win_size"]   = (bw, bh)
            except Exception:
                pass
            time.sleep(0.08)
