...
CHANNELS = {
    ...
    # TAKEOFF remains active_high (change if your hardware requires)
    "TAKEOFF": {"pin": 20, "active_level": 1, "feedback_pin": None},
        ...
        try:
            if cfg.get("active_level", 0) == 0:
                ...
                try:
                    ...
                    # fallback if PULL_DOWN not available
...
def pulse_channel(name, count=1):
    """Perform count pulses on channel 'name'. Each pulse: set output to active_level, wait, set to input (high-Z)."""
    ...
    # compute drive level: if active_level==0 -> drive 0, else 1
    ...
    for i in range(count):
        ...
        if not ok:
            success = False
            break
        # hold the active level
        time.sleep_ms(PULSE_ON_MS)
        # release (drive opposite / float): set back to high-Z with proper pull
        set_high_z(name)
